...
async def get_all_orders():
    ...
    while True:
        if first:
            ...
            params = {
                ...
                "created_at_min": since,
                "limit": 250,
                "fields": "id,name,created_at,line_items,shipping_address,financial_status,fulfillment_status,total_price,fulfillments,source_name,refunds"
            }
        else:
            params = {"limit": 250, "page_info": page_info,
                      "fields": "id,name,created_at,line_items,shipping_address,financial_status,fulfillment_status,total_price,fulfillments,source_name,refunds"}
    ...
    for o in all_orders:
        ...
        # Fulfilled item miqdarlarını hesabla
        fulfilled_qtys = {}  # line_item_id → fulfilled qty
        for f in o.get("fulfillments", []):
            if f.get("status") == "cancelled": continue
            for fi in f.get("line_items", []):
                lid = fi.get("id")
                fulfilled_qtys[lid] = fulfilled_qtys.get(lid,0) + fi.get("quantity",0)

        # Refund edilmiş item-lər
        refunded_qtys = {}
        for ref in o.get("refunds", []):
            for ri in ref.get("refund_line_items", []):
                lid = ri.get("line_item_id")
                refunded_qtys[lid] = refunded_qtys.get(lid,0) + ri.get("quantity",0)

        # Line items - status ilə
        items = []
        for li in o.get("line_items", []):
            lid = li.get("id")
            total_qty = li.get("quantity", 1)
            f_qty = fulfilled_qtys.get(lid, 0)
            r_qty = refunded_qtys.get(lid, 0)
            pending_qty = total_qty - f_qty - r_qty
